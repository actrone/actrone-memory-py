"""Compiled/type-checked docs examples for ``actrone-memory``.

CI type-checks this file with ``mypy --strict`` against the CURRENT package source, so an API
change that breaks a documented snippet fails the build. The ``# region`` blocks are extracted
verbatim into the docs by ``scripts/extract_snippets.py``, the guide never hand-types these, so
they cannot drift.
